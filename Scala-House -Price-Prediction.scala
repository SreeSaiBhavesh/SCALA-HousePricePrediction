...
// MAGIC val training = assembler.transform(train).select(col("features"), col("SalePrice").alias("label"))
// MAGIC training.show()
...
// MAGIC %scala
// MAGIC
// MAGIC val testing = assembler.transform(test).select(col("features"), col("SalePrice").alias("trueLabel"))
// MAGIC testing.show()
